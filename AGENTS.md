...
- CSV一括付与・既存ユーザー移行など「同じCSVを再実行しても二重付与されない」ことが
  要件になっている処理では、idempotencyKey に**バッチID (実行のたびに新規生成される値)
  を含めないこと**。CSV内容のハッシュなど、同一入力に対して常に同じ値になるものを使うこと。
  実装中、バッチIDを含めてしまい再実行時に二重付与されるバグを作り込んだ経緯がある。
- BigIntを含むレスポンスを返す可能性のあるコードを追加する場合は、
  `apps/api/src/common/bigint-json.ts` が読み込まれている経路 (`main.ts` 経由の起動、または
  `AppModule` を直接importするテスト) でのみ正しくJSONシリアライズされることに注意する。
