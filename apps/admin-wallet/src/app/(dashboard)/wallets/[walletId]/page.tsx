...
export default function WalletDetailPage() {
  ...
  if (!wallet && error) return <p className="p-6 text-sm text-sengoku-red">{error}</p>;
  if (!wallet) return <p className="p-6 text-sm text-sengoku-muted">読み込み中...</p>;
    ...
    <>
        ...
        <section className="mb-6 rounded-lg border border-sengoku-border bg-sengoku-navy p-4">
          ...
          <div className="flex flex-wrap items-end gap-3">
            ...
            <button onClick={() => runAction("grant")} className="rounded-md bg-sengoku-gold px-3 py-1.5 text-sm text-sengoku-navy-deep">
              付与
            </button>
            <button onClick={() => runAction("deduct")} className="rounded-md bg-sengoku-red px-3 py-1.5 text-sm text-white">
              減算
            </button>
            <button onClick={() => runAction("hold")} className="rounded-md bg-sengoku-navy px-3 py-1.5 text-sm text-white">
              保留
            </button>
          </div>
        ...
        <section className="mb-6 rounded-lg border border-sengoku-border bg-sengoku-navy p-4">
          ...
          {wallet.holds.length === 0 && <p className="text-xs text-sengoku-faint">保留はありません</p>}
