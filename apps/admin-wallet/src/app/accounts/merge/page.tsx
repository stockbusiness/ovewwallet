...
export default function AccountMergePage() {
  ...
  async function submit() {
    ...
    setError(null);
    try {
      const res = await apiFetch<MergeResult>("/api/v1/admin/accounts/merge", {
        method: "POST",
      ...
      setError(err instanceof ApiError ? err.message : "統合に失敗しました");
      setConfirming(false);
    }
  }
    ...
    <div>
      ...
      <main className="mx-auto max-w-2xl p-6">
        ...
        <p className="mb-4 text-xs text-neutral-500">
          ...
          統合元アカウントは MERGED 状態になりログインできなくなります。この操作は取り消せません。
        </p>
          ...
          {!confirming ? (
            ...
            <div className="rounded-md border border-red-200 bg-red-50 p-3">
              <p className="mb-2 text-sm text-red-700">
                本当に {sourceAccountCode} を {targetAccountCode} へ統合しますか? (取り消せません)
              </p>
        ...
        {result && (
          <p className="mt-3 rounded-md bg-emerald-50 p-3 text-sm text-emerald-700">
            統合が完了しました。移管残高: {Number(result.transferredAmount).toLocaleString("ja-JP")} OVE
          </p>
        )}
      </main>
    </div>
