          ...
          <tbody>
            {items.map((s) => (
              <tr key={s.id} className="border-t border-neutral-100">
                ...
                <td className="p-3">
                  <span className={s.status === "ACTIVE" ? "text-emerald-600" : "font-semibold text-red-600"}>
                    {s.status}
                  </span>
                </td>
                ...
                <td className="p-3">
                  {s.status === "ACTIVE" ? (
                    <button onClick={() => suspend(s.id)} className="text-xs text-red-600 underline">
                      緊急停止
                    </button>
                  ) : (
                    <button onClick={() => reactivate(s.id)} className="text-xs text-brand-600 underline">
                      再開
                    </button>
                  )}
                </td>
