...
import { EntitlementRevokedEventSchema } from "@ove/shared-types";
 ...
 */
type EntitlementRevokedBody = z.infer<typeof EntitlementRevokedEventSchema>;

@Injectable()
export class EntitlementRevokedHandler implements CommonEventHandler<EntitlementRevokedBody> {
  ...
  async handle(
    ...
    const reason =
      typeof metadata["reason"] === "string"
        ...
        : "entitlement.revoked";
    const reasonCode =
      typeof body.reason_code === "string" ? body.reason_code : null;
    const occurredAt = parseRequiredOccurredAt(
      body.occurred_at,
      "entitlement.revoked",
    );
    ...
    if (result.status === "source_conflict") {
      throw new ForbiddenException(
        `entitlement.revoked source mismatch for entitlement_id "${entitlementId}"`,
      );
    }
    ...
    return {
      action:
        result.status === "already_revoked" ? "already_revoked" : "revoked",
      holding_id: result.holding.id,
    };
  }
}
