...
| お知らせ管理 | `/notices` | ユーザー向けお知らせの作成 (公開日時の予約投稿込み)・公開・アーカイブ。公開中のお知らせは `GET /api/v1/me/notices` 経由でウォレットホーム・お知らせ一覧に表示される |
...
`/approval-requests`画面の「承認待ち」テーブルに「経過時間」列を追加した。
`requestedAt`とクライアント側の現在時刻から経過時間 (1時間未満/n時間/n日) を計算し、
24時間 (`SLA_HOURS`定数) 以上放置されている申請は行全体を赤く強調し「(放置)」と
表示する。バックエンドの変更は無く (`requestedAt`は元から返している)、既存データを
クライアント側で加工して表示するだけの純粋なUI機能。
