...
認証で弾かれた場合は、`x-api-key` と `Authorization: Bearer` の**両方**を試し、
どちらなら通るかを実測する。通常の送信経路は `x-api-key` 固定
(`IntegrationHttpClient` の `DEFAULT_API_KEY_HEADER`) だが、連携先がBearerしか
受けない場合、Flagを開けても本番の送信が同じ理由で失敗する。**開ける前に
ここで判別できるようにしてある。** Bearerでのみ通った場合は送信経路側の修正が要る。

| 結果 | 意味 |
|---|---|
| 疎通OK (x-api-key) | 送信先URLとAPIキーが正しい |
| 疎通OK (bearer) | キーは正しいが、**送信経路のヘッダーがBearerに対応していない**。要修正 |
| 401 | キーが認識されていない。登録値の不一致 |
| 403 | キーは認識されているが権限が無い可能性。連携先の応答本文を参照 |
| 404 | 送信先URLが違う |
| timeout / network | URLかネットワーク |

連携先が返した本文は先頭300文字だけ結果に含める (JSONの `error` メッセージか、
WAFのHTMLかで原因が変わるため)。
