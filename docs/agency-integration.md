...
ステップ2に **「接続テストを実行」** がある。保存済みの送信先URLとAPIキーで
`POST /api/common-users/resolve` を実際に呼び、**疎通と認証だけ**を確かめる。

- **Feature Flagを見ない** (`resolveAgencySystemConfigIgnoringFlag`)。Flagを開ける
  **前**に設定の正しさを確認できることが目的のため。実際の送信経路ではこの
  メソッドを使わないこと (Flagで止められなくなる)。
- 実在しないIDで `create_if_missing: false` を送るので、**相手側には何も残らない**。
- 共通顧客が見つからない応答も「疎通OK」。判定したいのは「URLが正しいか」
  「APIキーが通るか」の2点だけ。

| 結果 | 意味 |
|---|---|
| 疎通OK | 送信先URLとAPIキーが正しい |
| 401 | キーが認識されていない。登録値の不一致 |
| 403 | キーは認識されているが**権限が足りない**。連携先の応答本文を参照 |
| 404 | 送信先URLが違う |
| timeout / network | URLかネットワーク |

連携先が返した本文は先頭300文字だけ結果に含める。403のとき、どの権限が足りないのかが
ここに出る (2026-09-04の実例: `common_users:write` 不足)。

**このテストで確認できない範囲**: `create_if_missing: false` で問い合わせるため、
既存IDの参照までしか通らない。実際の登録経路は共通顧客IDの作成・更新を伴い、
`common_users:write` が要る。疎通OKでも登録経路が通るとは限らない点に注意。

APIキーの渡し方は `x-api-key` と `Authorization: Bearer` のどちらでも受け付けられる
(2026-09-04 連携先回答)。こちらの送信は `x-api-key` 固定
(`IntegrationHttpClient` の `DEFAULT_API_KEY_HEADER`) で問題ない。

本番の認証情報で外部へ発信するため、AUDITORには開放せず、実行を監査ログ
(`AGENCY_CONNECTION_TEST`) に残す。**鍵の生値は記録しない。**

なお**逆方向 (代理店 → ORI) はこちらからは試せない**。相手が実際に接続できたかは
ステップ3の `last_accessed_at` で確認する (代理店システム側にも接続テスト機能がある)。
