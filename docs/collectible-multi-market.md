...
## `sengoku-market` の帰属を移した (2026-09-06)

もともと `sengoku-market` は千ノ国NFTマーケットのLegacy表記として `nft-art-market` へ
寄せていた。これを**会員券の千ノ国マーケット側へ渡した。**

理由は、会員券マーケットが**全連携先で共通して使っている既存値がこれ**だと分かった
ため。先方は戦国パスポート・AIアート教室など他の連携先とも同じ値を使っており、
ウォレット向けだけ変えるのは避けたいとの回答だった。

**移せたのは、この時点で `sengoku-market` 由来の保有権が1件も無かったから。**
entitlementの受信を本番で開けたのが同日 (PR #85) で、かつ署名鍵を未発行だったため、
どのマーケットからも1件も届いていない。データを壊さずに入れ替えられる最後の機会だった。

千ノ国NFTマーケット側は正式値 `sennokuni-nft-market` の1本になる。契約v2指示書15章の
正式値そのもので、`sengoku-market` は元から旧称扱いだった。

**NFTマーケット側が `sennokuni-nft-market` を送っていることが前提。** まだ
`sengoku-market` を送っていると、そのカードは会員券マーケット扱いになり、種類が
合わないため400で弾かれる (データは壊れず、弾かれるだけ)。

## 会員券の千ノ国マーケット (2026-09-06 追加)

会員券を扱う千ノ国マーケット (`sengoku-commerce` および `sengoku-market`) を、
**論理Market `membership-market` として受け付ける**。2つの値を受け付けるのは、先方の
既存実装が `sengoku-market` を送り、文書上の正式値が `sengoku-commerce` であるため。NFTアートマーケット (`nft-art-market`) とは別の値にしてある。同じに
