...
## P0-2. LINE実チャネル結合試験 — 未着手 (設定一覧のみ整理)
...
| # | 項目 | 内容 |
|---|---|---|
| 1 | LINE Developersでのチャネル作成 | LINEログインチャネルを作成し、`channel_id`を取得する |
| 2 | `LINE_CHANNEL_ID`環境変数 | 上記チャネルIDを設定 (`.env.example`に既存項目あり) |
| 3 | コールバックURL登録 | LINE Developersコンソールで、ステージング/本番のコールバックURLを登録 |
| 4 | LIFF ID (LINE内ブラウザで開く場合) | `LINE_LIFF_ID`環境変数 (既存項目あり) |
| 5 | Cookieドメイン設定 | `COOKIE_DOMAIN`が実際のドメインと一致しているか確認 |
...
| P0-2 LINE実チャネル結合試験 | `CONFIGURATION_REQUIRED` (設定一覧を本書に整理済み、実施は未着手) |
