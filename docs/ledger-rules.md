...
`mergeAccounts()` (`packages/ledger/src/merge.ts`) は、上記の絶対ルールを保ったまま
「2つの異なるウォレット間で残高を移動する」唯一の関数。通常のCREDIT/DEBITが単一ウォレット
のみを対象とするのに対し、統合元・統合先の2つのウォレットをデッドロック回避のため
ID昇順で行ロックし、`ACCOUNT_MERGE_OUT` (統合元, DEBIT) と `ACCOUNT_MERGE_IN` (統合先,
CREDIT) の対になる取引を単一トランザクション内で作成する。両方とも通常のCOMPLETED取引
として記録されるため、指示書17章の整合性チェック式にも自然に含まれる (特別扱いは不要)。
詳細は `docs/admin-operations.md` の「アカウント統合」を参照。
