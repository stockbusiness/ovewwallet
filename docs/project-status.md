...
ウォレットホーム、取引履歴一覧 (獲得/利用/失効フィルタ)、取引詳細、メニュー、
連携サービス、貯める、使う、お知らせ一覧。共通コンポーネント (`packages/shared-ui`)
として切り出し済み。ダーク/ライト両テーマ対応 (CSS変数 + `ThemeToggle`)。
375px幅で全9画面確認済み、うち先行4画面 (ログイン/ウォレットホーム/取引履歴一覧/
取引詳細) は768px/1280pxでも確認済み。詳細: `docs/ui-design.md`
...
- 後発5画面 (メニュー・連携サービス・貯める・使う・お知らせ一覧) の768px/1280px確認、
  管理画面「お知らせ管理」の1280px確認は未実施 (`docs/ui-design.md`「レスポンシブ確認」参照)。
