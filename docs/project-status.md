# 独立OVEウォレット 実装状況整理 (2026-07-19時点)
...
(現時点で API 110件 + `packages/auth` 37件 + `packages/ledger` 27件 = 計174件、すべて成功)
...
ウォレットホーム (残高・保留内訳・階級表示・継続ログインボーナス・失効間近OVEの警告・
お知らせ)、
取引履歴一覧 (獲得/利用/失効フィルタ、CSVエクスポート)、取引詳細、メニュー
(紹介登録特典状況・退会を含む)、連携サービス、貯める、使う、お知らせ一覧、
ログイン中の端末一覧。共通コンポーネント (`packages/shared-ui`) として切り出し済み。
ダーク/ライト両テーマ対応 (CSS変数 + `ThemeToggle`)。375px幅で全10画面確認済み、
うち先行4画面 (ログイン/ウォレットホーム/取引履歴一覧/取引詳細) は768px/1280pxでも
...
- 後発6画面 (メニュー・連携サービス・貯める・使う・お知らせ一覧・ログイン中の端末) の
  768px/1280px確認、管理画面「お知らせ管理」の1280px確認は未実施
  (`docs/ui-design.md`「レスポンシブ確認」参照)。
- 2026-07-19フェーズ10で追加した10機能 (`docs/implementation-log.md`「フェーズ10」参照)
  はe2eテストに加え、同日中にPlaywrightによる実ブラウザ確認 (22件) も実施済み
  (`docs/implementation-log.md`「追記: 実ブラウザ確認」参照)。この過程で発見した
  ランク表示の`lifetime_credited`不整合バグは修正済み。ただし紹介登録の実捕捉フロー
  と複数端末同時ログインの一覧表示は未実施のまま (今後の課題)。
- 2026-07-19フェーズ11で追加した管理側7件・ウォレット側5件の計13機能
  (`docs/implementation-log.md`「フェーズ11」参照) はe2eテストで検証済みだが、
  実ブラウザでの動作確認は未実施 (今後の課題)。
