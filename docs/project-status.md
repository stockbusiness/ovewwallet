...
- 既存ユーザー移行: REVIEWINGアカウントの事後解消フロー、移行実行そのものの事前承認制・
  検証者(承認者)と実行者(申請者)の職務分離、さらに解消フロー側でも実行者本人による
  解消を禁止する職務分離まで対応済み (既存の二段階承認基盤・監査ログを再利用。
  `docs/migration.md` 「事前承認制・職務分離」「職務分離: 実行者本人による解消の禁止」
  参照)。文字コード対応 (Shift_JIS等) のみ未対応。
