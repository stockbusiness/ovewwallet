...
- 既存ユーザー移行: REVIEWINGアカウントの事後解消フローに加え、移行実行そのものの
  事前承認制・検証者(承認者)と実行者(申請者)の職務分離も対応済み (既存の二段階承認
  基盤を再利用。`docs/migration.md` 「事前承認制・職務分離」「検証者フロー」参照)。
  文字コード対応 (Shift_JIS等) のみ未対応。
...
- Playwright E2Eのリポジトリ内自動化: `tests/e2e`として土台を整備し、代表的な2フロー
  (ユーザーのLINEログイン→ウォレット表示、管理者の個別付与→残高反映) を自動化した
  (`pnpm test:e2e`)。アカウント統合の二段階承認・既存ユーザー移行の検証者フロー・
  管理者MFA等、他の多くのフローはまだ手動実行での確認のみ (`docs/test-plan.md`
  「Playwright E2Eのリポジトリ内自動化」参照)。
