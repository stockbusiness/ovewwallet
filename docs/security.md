...
- CSRF対策: `CsrfProtectionMiddleware` による更新系リクエストの検証
  (`Origin`が許可リスト外なら403、フォーム系Content-Typeは一律403)。
  CORSは応答の**読み取り**を防ぐ仕組みで、リクエスト自体の副作用は防げないため、
  CORSだけをCSRF対策と見なさないこと (`apps/api/src/common/csrf-protection.middleware.ts`)。
- ブルートフォース対策: メールOTPの試行回数上限(5回、メールアドレス単位)・再送間隔(60秒)。
  ...
  (下記「レート制限値の見直し」参照)。管理者ログインはこれに加えて**アカウント単位の
  失敗ロック**を課している (下記「管理者ログインの失敗ロック」参照)。
...
IPアドレス単位の制限だけでは、次の2つが素通りだった。

- **1アカウントへの分散総当たり**: IPを変えれば試行回数に上限が無い。特に2段階目は
  `mfaToken`が失敗しても消えないため、パスワードを知る相手はトークンの有効期間(5分)
  いっぱい、6桁のコードを複数IPから撃ち続けられた。
- **共有IPでの巻き添え**: 事務所など同一IPからの利用で、誰か1人の打ち間違いが
  他の管理者のログイン試行枠(60秒10回)を食い潰していた。

そこで `AdminLoginThrottleService` でアカウント単位の連続失敗を数え、上限に達したら
一定時間ロックする。IP制限と置き換えるのではなく併用する
(IP制限には「存在しないメールアドレスへの総当たり」を抑える役割が残るため)。

| 設定 | 既定 | 環境変数 |
|---|---|---|
| ロックまでの連続失敗回数 | 5回 | `ADMIN_LOGIN_MAX_FAILURES` |
| ロックの継続時間 | 15分 | `ADMIN_LOGIN_LOCK_SECONDS` |
| 失敗回数を数え続ける窓 | 15分 | (固定) |

- 1段階目(パスワード)と2段階目(TOTP)で回数を分けない。どちらも同じログイン試行の
  一部で、片方だけ数えると他方が抜け道になる。
- ロック中の試行はパスワード照合より**先に**弾き、その都度ロックを延長する
  (撃ち続ける相手に「待てば必ず開く」時刻を与えないため)。
- ロック中は正しいパスワード・正しいコードでも通さない。ここを通すと、総当たりで
  当たった瞬間に入られてしまい回数を数える意味が無くなる。
- ロックされていない間の応答は従来どおり401で変わらない (あと何回でロックされるかを
  教えない)。ロック時のみ429を返す。
- **存在しないメールアドレスはロックの対象にしない**。数える相手(adminId)が無く、
  ここでロックを返すと「そのアドレスの管理者が実在する」と教えることになるため、
  IP制限に任せる。
- **LINEログインは対象外**。認証はLINE側で行われ、こちらで数えられる「パスワードの
  失敗」が存在しない。
- 監査ログ (`ADMIN_LOGIN_LOCKED`, result=`FAILURE`) は**ロック時のみ**残す。失敗の
  たびに書くと、総当たりで`audit_logs`が膨れる (この表は削除できない)。

回数はKVストア (Redis) に持つ。管理者MFAのTOTP再利用防止と同じ基盤で、APIを複数
インスタンスで動かしても回数が共有される (プロセスごとに数えると、インスタンス数の
倍だけ試行できてしまう)。
...
**負荷テストで発見した問題**: `GET /health` に通常のAPIトラフィックと同じ60秒120回の
制限が適用されており、200並列リクエストの負荷テストで約4割が429になることが判明した
(`docs/test-plan.md` 「負荷・レート制限の限界値テスト」参照)。ヘルスチェックはk8sの
liveness/readinessプローブ等から高頻度でポーリングされる想定のため、`@SkipThrottle()`
を追加して通常のレート制限の対象から除外した。また、管理者ログインへの30並列リクエストで
`@nestjs/throttler` のカウントが真の同時実行下でも正確 (設定上限ちょうどで超過なし) で
あることを確認した。
