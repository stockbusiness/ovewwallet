...
| `specs/account-merge-approval.spec.ts` | アカウント統合の二段階承認: 申請者本人による承認が拒否されること・別の管理者が承認して初めて統合(残高移管)が実行されることを確認 |
| `specs/migration-approval.spec.ts` | 既存ユーザー移行の事前承認制・職務分離: 申請者本人による承認拒否・別管理者の承認で移行実行・REVIEWINGアカウントを移行実行者本人が解消しようとして拒否されること・別管理者による解消成功までを確認 |

**現時点では手動E2Eで確認した全フローを自動化したわけではない** (管理者MFA・外部連携キュー画面・
代理店連携状態一覧・紹介トークン受け入れ画面・レスポンシブ確認などは今回未着手。
`tests/e2e/README.md` の「今後の拡張候補」参照)。二段階承認まわり (アカウント統合・既存ユーザー
移行) は職務分離という他のフローより検証価値が高いと判断し優先して自動化した。
