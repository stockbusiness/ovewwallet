...
test.describe("admin-wallet: 既存ユーザー移行の事前承認制・検証者フロー", () => {
  ...
  test("requires a different admin to approve, then blocks the executor from resolving REVIEWING", async ({
    ...
    await expect(requesterPage.getByText("承認待ちとして申請しました。")).toBeVisible({ timeout: NAV_TIMEOUT_SHORT });
    ...
    await expect(approverPage.locator("tr", { hasText: batchName })).toContainText("APPROVED", { timeout: NAV_TIMEOUT_SHORT });
